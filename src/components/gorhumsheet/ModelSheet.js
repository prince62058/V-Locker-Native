...
const ModelSheet = forwardRef(({ selected, onSelect, handleSheetChanges }, ref) => {

    const { modelData } = useSelector(state => state.auth)
    ...
    const Option = ({ item }) => {
        const isSelected = selected?.modelName === item?.modelName
        ...
        >
            ...
            <BottomSheetFlatList
                ...
                contentContainerStyle={
                    (modelData?.data?.length ? styles.listContent : styles.emptyList)
                }
            />
        </BottomSheetModal>
    )
