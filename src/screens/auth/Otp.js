...
import { loginAction, sendOtp, verifyOtp } from '../../redux/slices/auth/authSlice'
...
const Otp = ({ navigation, route }) => {
    ...
    const handleResendOtp = () => {
        ...
        dispatch(sendOtp({phone: paramsData?.phone}))
    }
    ...
    const handleOtpPress = async() => {
        ...
        const response = await dispatch(verifyOtp({phone: paramsData?.phone, otpCode: otp.join('')}))

                if(verifyOtp.fulfilled.match(response)){
                    if(!response?.payload?.userData?.isProfileCompleted){
            ...
            >
                ...
                >
                    ...
                    <MainText style={styles.desc}>
                        {`We’ve sent a 4-digit code to +91 •••••• ${paramsData?.phone?.slice(6, 10)}`}
                    </MainText>
