...
import { View, Text, StyleSheet, Dimensions, Animated, Easing, Platform, NativeModules } from 'react-native';
                    ...
                    <Animated.View style={{ opacity: 1 /* can allow fade if needed */ }}>
                        ...
                        */}
                        ...
                        >
                            ...
                            <View style={{ overflow: 'hidden', ...StyleSheet.absoluteFillObject, borderRadius: 16 }}>
                                <Animated.View
                                    style={{
                                        width: LOCK_BODY * 0.4,
                                        height: '140%',
...
//             Animated.sequence([
//                 Animated.delay(delay),
//                 Animated.timing(o, { toValue: 0.45, duration: 260, useNativeDriver: true })
//             ]).start();
//         }, []);
//         return (
//             <Animated.View
...
//                     {/* Rivets around body perimeter */}
//                     <Rivet x={width * 0.45} y={height * 0.42} />
//                     <Rivet x={width * 0.55} y={height * 0.42} delay={160} />
//                     <Rivet x={width * 0.45} y={height * 0.58} delay={180} />
//                     <Rivet x={width * 0.55} y={height * 0.58} delay={200} />

//                     {/* Lock body (gradient disc) */}
//                     <Animated.View
//                         style={[
//                             styles.lockBody,
//                             {
...
//                     {/* Shackle: ring rotates then morphs to U, sits above body */}
//                     <Animated.View
//                         style={[
//                             styles.shackleWrapper,
//                             {
//                                 width: SHACKLE_W,
//                                 height: SHACKLE_H,
//                                 transform: [
...
//                                     { scaleX: shackleOpen },
//                                 ],
//                                 borderRadius: shackleRadius,
//                                 borderWidth: SHACKLE_T,
//                                 borderColor: COLORS?.primary ?? '#3E8EFF',
//                             },
//                         ]}
//                     >
//                         {/* initial rotating ring visual underneath during pre-morph */}
//                         <Animated.View
//                             style={[
//                                 styles.rotateRing,
//                                 {
//                                     width: LOCK_BODY * 0.98,
//                                     height: LOCK_BODY * 0.98,
//                                     borderRadius: 999,
//                                     transform: [{ rotate: rotateDeg }],
//                                     opacity: morph.interpolate({ inputRange: [0, 1], outputRange: [1, 0] }),
//                                     borderWidth: SHACKLE_T * 0.9,
//                                     borderColor: '#8A6CFF',
//                                 },
//                             ]}
//                         />
...
//                             {
//                                 width: LOCK_BODY * 0.9,
//                                 height: LOCK_BODY * 0.9,
//                                 borderRadius: (LOCK_BODY * 0.9) / 2,
//                                 opacity: glowOpacity,
//                                 shadowOpacity: Platform.OS === 'ios' ? 0.9 : 0,
//                                 shadowColor: COLORS?.primary ?? '#3E8EFF',
//                             },
//                         ]}
//                     />
//                 </View>

//                 {/* Final expanding radial reveal */}
//                 <Animated.View
//                     style={[
//                         styles.revealCircle,
//                         {
...
function computeMaxScale(bodyWidth) {
    const rNeeded = Math.sqrt(Math.pow(width / 2, 2) + Math.pow(height / 2, 2));
    const baseRadius = bodyWidth / 2;
    const s = rNeeded / baseRadius;
    return s * 1.1;
}
...
const Splash = ({ onFinish }) => {
    const BASE = Math.min(width, height);
    // Lock proportions
    const BODY_W = BASE * 0.44;
    const BODY_H = BODY_W * 0.78;           // slightly shorter than square
    const BODY_RADIUS = BODY_W * 0.22;      // bottom corner radius
    const SHACKLE_THICK = Math.max(4, BODY_W * 0.10);
    const SHACKLE_INNER_W = BODY_W * 0.56;  // inner width of U
    const SHACKLE_HEIGHT = BODY_H * 0.62;   // total shackle height above body
    const SHACKLE_GAP = BODY_W * 0.08;      // gap from body top to shackle inner
    const KEYHOLE_C = BODY_W * 0.16;        // keyhole circle diameter
    const KEYHOLE_STEM_W = KEYHOLE_C * 0.36;
    const KEYHOLE_STEM_H = KEYHOLE_C * 1.15;

    // Timings (<= 3000ms total)
    const T_LETTERS = 420;
    const T_SHACKLE = 600;
    const T_BODY = 480;
    const T_KEYHOLE = 320;
    const T_REVEAL = 900;
    const T_DISSOLVE = 340;
    ...
    const shackleDrop = useRef(new Animated.Value(-30)).current;
    const shackleOpacity = useRef(new Animated.Value(0)).current;
    const bodyScale = useRef(new Animated.Value(0.85)).current;
    const bodyOpacity = useRef(new Animated.Value(0)).current;

    const keyholeOpacity = useRef(new Animated.Value(0)).current;
    const keyholeScale = useRef(new Animated.Value(0.8)).current;

    const targetScale = useMemo(() => computeMaxScale(BODY_W), [BODY_W]);
    ...
    useEffect(() => {
        ...
        // Shackle drop-in
        const shackleIn = Animated.parallel([
        ...
        // Body pop
        const bodyIn = Animated.parallel([
        ...
        // Keyhole appear
        const keyholeIn = Animated.parallel([
            Animated.timing(keyholeOpacity, {
                toValue: 1,
                duration: T_KEYHOLE,
                easing: Easing.out(Easing.cubic),
                useNativeDriver: true,
            }),
            Animated.spring(keyholeScale, {
                toValue: 1,
                stiffness: 220,
                damping: 16,
                mass: 0.7,
        ...
        // Reveal + dissolve
        const reveal = Animated.timing(revealScale, {
            toValue: targetScale,
            duration: T_REVEAL,
            easing: Easing.out(Easing.cubic),
            useNativeDriver: true,
        ...
        Animated.sequence([
            Animated.stagger(45, letterAnims),                 // ~500ms
            Animated.parallel([shackleIn, bodyIn]),            // overlaps, ~600ms
            keyholeIn,                                         // ~320ms
            Animated.parallel([reveal, dissolve]),             // ~900ms
        ]).start(() => {
            if (mounted && onFinish) onFinish();
        });
    ...
    // Helper components (pure views with animated transforms)
    const LockBody = () => (
        <Animated.View
            style={[
                styles.lockBody,
                {
                    ...
                    opacity: bodyOpacity,
                    transform: [{ scale: bodyScale }],
                    borderTopLeftRadius: BODY_RADIUS * 0.35,
                    borderTopRightRadius: BODY_RADIUS * 0.35,
                    borderBottomLeftRadius: BODY_RADIUS,
                    borderBottomRightRadius: BODY_RADIUS,
                },
            ]}
        >
            <LinearGradient
    ...
    const Shackle = () => {
        const OUTER_W = SHACKLE_INNER_W + SHACKLE_THICK * 2;
        const OUTER_H = SHACKLE_HEIGHT;
        const OUTER_R = OUTER_W / 2;
            ...
            <Animated.View
                style={[
                    styles.shackle,
                    {
                        ...
                        borderWidth: SHACKLE_THICK,
                        opacity: shackleOpacity,
                        transform: [{ translateY: shackleDrop }],
                        borderColor: COLORS?.primary ?? '#3E8EFF',
                    },
                ]}
            >
                {/* Mask the bottom of the shackle to create a flat open end */}
                <View
                    pointerEvents="none"
                    style={{
                        position: 'absolute',
                        left: SHACKLE_THICK * 0.6,
                        right: SHACKLE_THICK * 0.6,
                        bottom: -SHACKLE_THICK * 0.55,
                        height: SHACKLE_THICK * 2,
                        backgroundColor: '#0d0f14',
                        borderRadius: SHACKLE_THICK,
                    }}
                />
            </Animated.View>
        );
        ...
        >
            <View
                style={{
                    width: KEYHOLE_C,
                    height: KEYHOLE_C,
                    borderRadius: KEYHOLE_C / 2,
                    backgroundColor: '#0d0f14',
                }}
            />
            <View
                style={{
                    width: KEYHOLE_STEM_W,
                    height: KEYHOLE_STEM_H,
                    borderRadius: 999,
                    marginTop: -KEYHOLE_C * 0.12,
                    backgroundColor: '#0d0f14',
                }}
            />
        </Animated.View>
...
const styles = StyleSheet.create({
    ...
    shackle: {
        position: 'absolute',
        top: height / 2 - (Math.min(width, height) * 0.44) * 0.78 / 2 - (Math.min(width, height) * 0.44) * 0.62 + 6,
        alignItems: 'center',
        justifyContent: 'flex-start',
        backgroundColor: 'transparent',
    },
    ...
    keyholeWrap: {
        position: 'absolute',
        alignSelf: 'center',
        top: height / 2 - (Math.min(width, height) * 0.44) * 0.78 / 2 + (Math.min(width, height) * 0.44) * 0.22,
        alignItems: 'center',
    },
    keyhole: {
        alignItems: 'center',
