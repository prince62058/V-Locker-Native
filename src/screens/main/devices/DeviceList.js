...
import { clearSearch, getLoanListThunk } from '../../../redux/slices/main/loanSlice'
...
const DeviceList = ({ navigation, route }) => {
    ...
    const { loanData, loading, pagination, searchData, searchPagination } = useSelector(state => state.loan)
    // console.log('Loan devices ---> ', loading)

    const [search, setSearch] = useState('')

    const fetchData = useCallback(({ isRefresh = false, page = 1, search = '', filter = key }) => {
    ...
    const handlePagination = () => {
        ...
        if (loading?.pagination || loading?.loading || searchPagination?.loading || searchPagination?.pagination) return
