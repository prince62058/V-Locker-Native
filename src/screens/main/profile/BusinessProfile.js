...
import { getBusinessThunk, updateBusinessThunk } from '../../../redux/slices/main/businessSlice'

const BusinessProfile = ({ navigation }) => {

    const dispatch = useDispatch()
    const { businessData, loading, updating } = useSelector(state => state.business)
            ...
            >
                <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'space-between' }}>
                    ...
                    <Pressable style={styles.profileUrl} onPress={handleImageSelect}>
                        {form.profileUrl && (
                            <Image source={{ uri: form?.profileUrl?.uri || form?.profileUrl }} style={{ width: '100%', height: '100%' }} resizeMode='cover' />
                        )}
                    </Pressable>
